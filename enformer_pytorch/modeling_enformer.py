...
#     return (pred - target * log(pred + 1e-6)).mean()

# This prevents log(0) and very large magnitude values from tiny preds. Safe and recommended.
def poisson_loss(pred, target, eps=1e-8):
    pred = pred.clamp(min=eps)               # avoid log(0) or extreme negative logs
